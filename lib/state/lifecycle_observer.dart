...
class _LifecycleObserverState extends ConsumerState<LifecycleObserver> with WidgetsBindingObserver {
  ...
  Future<void> _onAppToForeground() async {
    ...
    await prefs.setInt(_lastDataFetchTimestampKey, DateTime.now().millisecondsSinceEpoch);
  }
  ...
  Future<void> _checkBackgroundKill() async {
    ...
    if (wasInBackground && lastBackgroundTime != null) {
      ...
      final timeSinceBackground = DateTime.now().millisecondsSinceEpoch - lastBackgroundTime;
    ...
    try {
      ...
      if (mounted) {
        ...
        if (currentRoute.isNotEmpty) {
          ...
          await prefs.setInt(_lastRouteTimestampKey, DateTime.now().millisecondsSinceEpoch);
  ...
  Future<void> _restoreAppState() async {
    ...
    try {
      ...
      if (lastRoute != null && lastRouteTimestamp != null) {
        final timeSinceRouteCapture = DateTime.now().millisecondsSinceEpoch - lastRouteTimestamp;
        
        // Only restore if route was captured recently (within 5 minutes)
        if (timeSinceRouteCapture < 300000 && mounted) { // 5 minutes
          final router = GoRouter.of(context);
          final currentRoute = router.routerDelegate.currentConfiguration.last.matchedLocation;
  ...
  Future<bool> _shouldRefreshData() async {
    ...
    final timeSinceLastFetch = DateTime.now().millisecondsSinceEpoch - lastFetchTimestamp;
